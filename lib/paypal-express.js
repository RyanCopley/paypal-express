...
PaypalExpress.prototype.beginInstantPayment = function (price, invoiceNumber, options, callback) {
    ...
    this.nvpreq.makeRequest(options, function (err, data) {
        ...
        if (response.ACK === 'Success') {
            ...
            callback(null, response.TOKEN, self.buildCheckoutUrl(response.TOKEN));
...
PaypalExpress.prototype.validateIPN = function (response, callback) {

    var req_options = {
        host: (this.sandbox) ? SANDBOX_URL : REGULAR_URL,
        path: '/cgi-bin/webscr?' + qs.stringify(response) + '&cmd=_notify-validate',
        method: 'GET'
    }
